...
selector?.addEventListener("change", () => {
  ...
  if (container.children.length >= 5) {
    alert("Maximum 5 badges par plat");
    return;
  }

  const alreadyExists = [...container.querySelectorAll("input")]
    .some(input => input.value === selector.value);
...
document.getElementById("createTagBtn")?.addEventListener("click", async () => {
  const name = prompt("Nom du badge (5 mots max)");
  if (!name) return;
...
document.querySelectorAll(".delete-tag-global")
  .forEach(btn => {
